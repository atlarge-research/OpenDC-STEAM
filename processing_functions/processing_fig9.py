...
with open(f"{base_folder}/aggregated/{workload}/peak_powers.csv", "w") as f:
    f.write("Name,Peak Power (kW)\n")
    for name, peak_power in peak_powers.items():
        f.write(f"{name},{peak_power}\n")
        
with open(f"{base_folder}/aggregated/{workload}/total_energies.csv", "w") as f:
    f.write("Name,Total Energy (kWh)\n")
    for name, total_energy in total_energies.items():
        f.write(f"{name},{total_energy}\n")

with open(f"{base_folder}/aggregated/{workload}/task_delays.csv", "w") as f:
